...
class FitTrack(QWidget):
    ...
    def initUI(self):
        ...
        self.table.setHorizontalHeaderLabels(["Date","Calories","Distance","Description"])
    ...
    def load_table(self):
        ...
        while query.next():
            ...
            self.table.setItem(row, 1, QTableWidgetItem(str(calories)))
            self.table.setItem(row, 2, QTableWidgetItem(str(distance)))
    ...
    def delete_workout(self):
        ...
        fit_id = (self.table.item(selected_row,1).text())
        ...
        query = QSqlQuery()
        query.prepare("DELETE FROM fitness WHERE calories = ?")
    ...
    def calculate_calories(self):
        ...
        try:
            ...
            ax.scatter(distances, calories, c=normalized_calories, cmap="Wistia", label="Data Points")
            ax.set_title("Distance Vs. Calories")
            ax.set_xlabel("Distance")
            ax.set_ylabel("Calories")
            cbar = ax.figure.colorbar(ax.collections[0], label="Calories Burn Index")
            ax.legend()
            self.canvas.draw()
